		...
		*/
        render: function(dst, trees, opt) {
            ...
            if (typeof opt.theme === 'string' && opt.theme !== '') {
                ...
                layui.link(layui.cache.base+'res/css/component/authtree/'+opt.theme+'.css');
            }
            if (opt.hidechoose) {
            ...
            var timer = 0;
            $(dst).find('.auth-single:first').unbind('click').on('click', '.layui-form-checkbox,.layui-form-radio',
            function(event) {
            ...
            /*动态绑定展开事件*/
            $(dst).unbind('click').on('click', '.auth-icon',
            function() {
                obj.iconToggle(dst, this);
            });
            /*双击展开*/
            $(dst).find('.auth-single:first').unbind('dblclick').on('dblclick', '.layui-form-checkbox,.layui-form-radio',
            function(e) {
                // 触发时间 > 0，才触发双击事件
        ...
        renderAuth: function(tree, dept, opt) {
            ...
            function(index, item) {
                ...
                str += '<div class="auth-skin"><div class="auth-status" style="display: flex;flex-direction: row;align-items: flex-end;"> ' + (hasChild ? '<i class="layui-icon auth-icon ' + (openstatus ? 'active': '') + '" style="cursor:pointer;">' + (openstatus ? obj.openIconContent: obj.closeIconContent) + '</i>': '<i class="layui-icon auth-leaf" style="opacity:0;color: transparent;">&#xe626;</i>') + (dept > 0 ? ('<span>' + opt.prefixChildStr + ' </span>') : '') + '<input class="authtree-checkitem" type="' + opt.checkType + '" name="' + inputname + '" title="' + item[nameKey] + '" value="' + item[valueKey] + '" lay-skin="primary" lay-filter="' + layfilter + '" ' + (isChecked ? ' checked="checked"': '') + (isDisabled ? ' disabled': '') + '> </div>' + ' <div class="auth-child" style="' + (openstatus ? '': 'display:none;') + 'padding-left:40px;"> ' + append + '</div></div>'
            });
            str += '</div>';
            return str;
        },
